...
class ResNeXt50_32x4d(nn.Module):
    ...
    def __init__(self, in_channels, n_classes):
        ...
        self.conv2 = nn.Sequential(
            ...
            self._make_layers(
                in_channels=64, out_channels=256, groups=32, stride=1, num_layers=3
            ),
        )
        self.conv3 = self._make_layers(
            in_channels=256, out_channels=512, groups=32, stride=2, num_layers=4
        )
        self.conv4 = self._make_layers(
            in_channels=512, out_channels=1024, groups=32, stride=2, num_layers=6
        )
        self.conv5 = self._make_layers(
            in_channels=1024, out_channels=2048, groups=32, stride=2, num_layers=3
        )
    ...
    def _make_layers(self, in_channels, out_channels, groups, stride, num_layers):
        layers = []
        layers.append(
            ResNextBlock(
                in_channels=in_channels,
